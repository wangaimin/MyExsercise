using YZ.JsonRpc.ExtensionInject;
    ...
    public class Handler
    {
        ...
        public JsonResponse Handle(JsonRequest Rpc, Object RpcContext = null, Action<JsonResponse> callback = null)
        {
            ...
            if (haveDelegate == false || haveMetadata == false || metadata == null || handle == null)
            {
                ...
                {
                    Result = null,
                    Error = new JsonRpcException(-32601, "Method not found", "The method " + Rpc.Method + " does not exist / is not available.", Rpc),
                    Id = Rpc.Id
                };
                callback.Invoke(response);
                CompletedProcess(Rpc, response, RpcContext);
            ...
            catch (Exception ex)
            {
                ...
                {
                    response = new JsonResponse() { Error = ProcessException(Rpc, new JsonRpcException(-32602, "Invalid params", ex, Rpc)) };
                    callback.Invoke(response);
                    CompletedProcess(Rpc, response, RpcContext);
                    return response;
                }
                ...
                {
                    response = new JsonResponse() { Error = ProcessException(Rpc, new JsonRpcException(-32603, "Internal Error", ex.InnerException, Rpc)) };
                    callback.Invoke(response);
                    CompletedProcess(Rpc, response, RpcContext);
                    return response;
                }

                response = new JsonResponse() { Error = ProcessException(Rpc, new JsonRpcException(-32603, "Internal Error", ex, Rpc)) };
                callback.Invoke(response);
                CompletedProcess(Rpc, response, RpcContext);
                return response;
            }
        ...
        {
            new JsonRpcInject().CompletedProcess(request, response);
